...
class PPO_trainer():
    def __init__(self, discriminator, num_parralel_envs, num_total_timesteps, num_steps, gae_lambda, num_minibatches=4,learning_rate=1e-3, action_dim=4, anneal_lr=False, multiprocess=False, gamma=0.95, epochs=4, clip_coef=0.1, clip_vloss=True, entropy_coef=0.05, value_coef=0.5, max_grad_norm=0.5, terminal_confidence=0.95):
        #training params
    ...
    def train(self, discriminator):
        ...
        for update_num in range(self.num_updates):
            ...
            for step in range(self.num_steps):
                ...
                with torch.no_grad():
                    move, logprob, _ = self.agent.get_action_and_move(next_obs)
                    value = self.agent.get_value(next_obs)
                ...
                next_obs, reward, done, _ = self.envs.step(action.cpu().numpy())
                self.rewards[step] = torch.tensor(reward).to(self.device).view(-1)
                next_obs, next_done = torch.Tensor(next_obs).to(self.device), torch.Tensor(done).to(self.device)     
