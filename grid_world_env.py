...
class GridWorldEnv(gym.Env):
    ...
    def __init__(self, 
        ...
        if action_type == 'only_explore':
            ...
            self.action_space = gym.spaces.Discrete(4+1000)
        if ob_type == 'local':
            # (pixel value, height index, width index)
            # observation space is left and right inclusive
            self.observation_space = gym.spaces.Box(low=np.array([0, 0, 0]), high=np.array([1, 27, 27]), dtype=np.uint8)
        elif ob_type == 'global':
            self.observation_space = gym.spaces.Box(low=np.zeros((1, 28, 28)), high=np.full((1, 28, 28), 255), dtype=np.uint8)
        else:
            raise TypeError
    ...
    def reset(self):
        ...
        self.discover = True
        self.img_belief = np.full((1, 28, 28), mu.unexplored, dtype=np.uint8)
